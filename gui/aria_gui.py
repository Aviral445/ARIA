...
class AriaApp:
    ...
    def _build_layout(self):
        ...
        self.sidebar = tk.Frame(self.root, bg=self.pal["BG_MID"], width=340)
    ...
    def _build_nav_rail(self):
        ...
        # Top Logo Badge
        lc = tk.Canvas(n, width=56, height=56, bg=pal["BG_PANEL"], highlightthickness=0)
        lc.pack(pady=(8, 4))
        lc.create_oval(14, 14, 42, 42, fill=blend(pal["GLOW"], pal["BG_PANEL"], 0.7), outline=pal["GLOW"], width=1)
        ...
        for icon, pid, label in self._nav_items:
            c = tk.Canvas(n, width=56, height=44, bg=pal["BG_PANEL"], highlightthickness=0, cursor="hand2")
            c.pack(pady=1)
        ...
        self.nav_dot_c = tk.Canvas(n, width=56, height=40, bg=pal["BG_PANEL"], highlightthickness=0)
        self.nav_dot_c.pack(side="bottom")
        self.nav_dot_c.create_oval(22, 6, 34, 18, fill=pal["DGREY"], outline="", tags="dot")
        self.nav_dot_c.create_text(28, 28, text="IDLE", font=("Segoe UI", 7, "bold"), fill=pal["GREY"], tags="lbl")

        # Theme Cycle Button
        theme_btn = tk.Button(
            n, text="🎨", font=("Segoe UI", 10), bg=pal["BG_PANEL"], fg=pal["LAVENDER"],
    ...
    def _draw_nav_btn(self, c: tk.Canvas, icon: str, label: str, pid: str, active: bool = False, hover: bool = False):
        ...
        c.delete("all")
        if active:
            c.create_rectangle(0, 0, 56, 44, fill=blend(pal["GLOW"], pal["BG_PANEL"], 0.85), outline="")
            c.create_rectangle(0, 4, 3, 40, fill=pal["GLOW"], outline="")
        elif hover:
            c.create_rectangle(0, 0, 56, 44, fill=blend(pal["GLOW"], pal["BG_PANEL"], 0.93), outline="")
        else:
            c.create_rectangle(0, 0, 56, 44, fill=pal["BG_PANEL"], outline="")

        col = pal["WHITE"] if active else (pal["LAVENDER"] if hover else pal["GREY"])
        c.create_text(28, 16, text=icon, font=("Segoe UI", 12), fill=col)
        c.create_text(28, 32, text=label, font=("Segoe UI", 6, "bold"), fill=col)
    ...
    def _build_sidebar(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="ARIA PRO", font=("Segoe UI", 16, "bold"), bg=pal["BG_MID"], fg=pal["WHITE"]).pack(side="left")
        
        badge = tk.Frame(hdr, bg=pal["CARD2"], padx=6, pady=2)
        badge.pack(side="right")
        tk.Label(badge, text="v5.0 SINGULARITY", font=("Segoe UI", 7, "bold"), bg=pal["CARD2"], fg=pal["CYAN"]).pack()

        # 2. Black Hole Singularity Canvas (220px)
        self.sc = tk.Canvas(s, width=320, height=210, bg=pal["BG_MID"], highlightthickness=0)
        ...
        self.status_text = tk.Label(self.status_pill, text="STANDBY // IDLE", font=("Segoe UI", 8, "bold"), bg=pal["CARD2"], fg=pal["WHITE"])
        self.status_text.pack(side="left")

        # 3. Master Launch / Stop Button
        self.start_btn = tk.Button(
            s, text="▶   LAUNCH ARIA AGENT", font=("Segoe UI", 10, "bold"),
        ...
        tk.Label(t_hdr, text="SYSTEM TELEMETRY", font=("Segoe UI", 7, "bold"), bg=pal["CARD"], fg=pal["GREY"]).pack(side="left")
        self.ping_lbl = tk.Label(t_hdr, text="24ms", font=("Segoe UI", 7, "bold"), bg=pal["CARD"], fg=pal["CYAN"])
        ...
        self.quick_cmd_entry = tk.Entry(
            cmd_box, font=("Segoe UI", 9), bg=pal["CARD2"], fg=pal["WHITE"],
        ...
        tk.Button(
            cmd_box, text="➤", font=("Segoe UI", 9, "bold"),
        ...
        s_hdr.pack(fill="x")
        tk.Label(s_hdr, text="📱 MOBILE LAN COMPANION", font=("Segoe UI", 7, "bold"), bg=pal["CARD"], fg=pal["CYAN"]).pack(side="left")
        self.srv_state_lbl = tk.Label(s_hdr, text="● ONLINE", font=("Segoe UI", 7, "bold"), bg=pal["CARD"], fg=pal["GREEN"])
        ...
        self.srv_url_text = tk.Label(
            ...
            font=("Segoe UI", 8, "bold"), bg=pal["CARD"], fg=pal["WHITE"], cursor="hand2"
        )
        ...
        self.toggle_srv_btn = tk.Button(
            s_btns, text="TOGGLE", font=("Segoe UI", 7, "bold"), bg=pal["BG_MID"], fg=pal["LAVENDER"],
        ...
        tk.Button(
            s_btns, text="COPY", font=("Segoe UI", 7, "bold"), bg=pal["BG_MID"], fg=pal["LAVENDER"],
            relief="flat", bd=0, pady=3, padx=6, cursor="hand2", command=self._copy_mobile_server_url
        ).pack(side="left", padx=2)

        tk.Button(
            s_btns, text="OPEN", font=("Segoe UI", 7, "bold"), bg=pal["BG_MID"], fg=pal["CYAN"],
    ...
    def _mini_stat(self, parent, row: int, col: int, title: str, val: str, accent: str):
        ...
        tk.Label(f, text=title, font=("Segoe UI", 6, "bold"), bg=pal["CARD2"], fg=pal["GREY"]).pack(anchor="w")
        lbl = tk.Label(f, text=val, font=("Segoe UI", 8, "bold"), bg=pal["CARD2"], fg=accent)
        lbl.pack(anchor="w")
        return lbl
    ...
    def _pg_chat(self):
        ...
        for chip_text in ["Inspect Screen", "List Open Windows", "NVIDIA RPM Stats", "Summarize Clipboard"]:
            btn = tk.Button(
                ctrl_bar, text=chip_text, font=("Segoe UI", 7, "bold"),
        ...
        self.chat_main_entry = tk.Entry(
            in_bar, font=("Segoe UI", 10), bg=pal["CARD2"], fg=pal["WHITE"],
            insertbackground=pal["CYAN"], relief="flat", bd=0
        )
    ...
    def _append_chat_message(self, role: str, text: str):
        ...
        tk.Label(hdr, text=sender, font=("Segoe UI", 8, "bold"), bg=bubble_bg, fg=tag_col).pack(side="left")
        tk.Label(hdr, text=f" {time.strftime('%I:%M %p')}", font=("Segoe UI", 7), bg=bubble_bg, fg=pal["GREY"]).pack(side="left")

        # Formatted Body Text
        msg_lbl = tk.Label(
            bubble, text=text, font=("Segoe UI", 9), bg=bubble_bg, fg=pal["WHITE"],
